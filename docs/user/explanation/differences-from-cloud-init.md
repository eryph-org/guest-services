...
| Module list | Many cloud-init modules; selected by `cloud.cfg` lists | A fixed catalog: `Growpart`, `SetHostname`, `ApplyNetworkConfig`, `NtpClient`, `Timezone`, `SetLocale`, `UsersGroups`, `SetPasswords`, `SshModule`, `WriteFiles`, `WriteFilesDeferred`, `Runcmd`, `Licensing`, `ScriptsUser`, `PowerState` (selectable per stage — see the configurable-module-lists row below) | Linux-only and deferred top-level keys (`apt`, `snap`, `packages`, `bootcmd`, `phone_home`, `chef`, …) are **accepted** in the schema and logged at Info by `CloudConfigSerializer`'s acknowledged-key inventory so cross-cloud YAML round-trips cleanly. The inventory is source-generated from the model's `[CloudInitField(Platforms = …)]` attributes — adding a new Linux-only key needs no parallel list update. `egs-service validate --target windows` surfaces the same data per file. See [RFC 0028](../../rfcs/0028-linux-keys-module.md). |
...
| Random passwords (`chpasswd.users[].type: RANDOM`, `chpasswd.list` `R`/`RANDOM`, or a password-less `chpasswd.users` entry) | Generates a password and delivers it out-of-band by writing it to `/dev/console` (surfaced via the platform console log) | **Not supported.** Windows guests have no `/dev/console` analogue reliably captured across the clouds eryph targets, so a generated password could never be retrieved — setting one would silently lock the operator out. `egs-service validate` **rejects** these; at runtime they are **warn-and-skipped**. The schema still parses the keys (round-trip). Specify an explicit password instead. |
...
| Configurable per-stage module lists | `cloud_init_modules`, `cloud_config_modules`, `cloud_final_modules` in `cloud.cfg` | **Implemented** — the `stages` settings block carries a per-stage `enabledModules` / `disabledModules` allow/deny list (case-insensitive, tolerates the `Module` suffix). When a stage is absent, all discovered modules for it run. Stage membership and intra-stage ordering remain fixed by `[Stage]` / `[Order]` attributes. | Mirrors cloud-init's `cloud_*_modules` selection without re-implementing its free-form ordering. See [RFC 0009](../../rfcs/0009-module-list-split.md). |
| Configurable datasource list | `datasource_list` in `cloud.cfg` selects/orders which datasources are probed | **Implemented** — `dataSources.dataSourceList` names the sources to probe, **in the listed order** (case-insensitive on the source name). When null/empty, all registered sources are probed in `Priority` order; unknown names are logged at Warning and skipped. | Mirrors cloud-init's `datasource_list`. |
