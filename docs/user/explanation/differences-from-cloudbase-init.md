...
| Stages | OOBE / pre-networking / main / post-sysprep — Python plugin chain | Cloud-init's Local / Network / Config / Final shape | Keeps the cloud-init mental model. |
...
| Random password surface | Available via plugin chain | **Not supported** — `type: RANDOM`, `chpasswd.list` `R`/`RANDOM`, and password-less entries are rejected by `validate` and warn-skipped at runtime | cbi surfaces the generated value via the console; Windows guests have no `/dev/console` analogue reliably captured across the clouds eryph targets, so a generated password could never be retrieved. Set an explicit password. |
...
| Locale / keyboard | No equivalent — operators run `Set-Culture` / `Set-WinUserLanguageList` via runcmd | `locale:` / `keyboard.layout:` cloud-config keys driving `Set-Culture` / `Set-WinUILanguageOverride` / `Set-WinUserLanguageList` / `Set-WinSystemLocale` | Common operator customisation; cbi gap. Reboot only requested when `Set-WinSystemLocale` actually changed. See [RFC 0027](../../rfcs/0027-set-locale-module.md). |
