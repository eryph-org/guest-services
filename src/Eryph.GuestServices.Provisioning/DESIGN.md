...
- `DataSources/` — `IDataSource`, `NoCloudDataSource` (reads `meta-data` + `user-data` from a mounted volume labelled `cidata`/`config-2` or from a configured path), `ConfigDriveDataSource` (OpenStack config-2 layout off a mounted disk), `OpenStackMetadataDataSource` (the same OpenStack `openstack/<version>/` layout fetched over the HTTP metadata service at `169.254.169.254`; shares `OpenStack/OpenStackMetadataReader` with ConfigDrive), `KvpDataSource` stub (reads from the existing KVP guest plumbing).

  **Maturity — `OpenStackMetadataDataSource` is "technically working", not production-ready.** It is validated against the real captured nova fixture (`test/fixtures/configdrive-openstack`) over both transports (unit tests) and end-to-end on eryph/Hyper-V against the `egs-openstack-sim` simulator serving that fixture — including a login user provisioned from `user_data`. It has **never been run against a real OpenStack deployment**, where reachability of the link-local metadata IP differs (neutron metadata agent on-link / DHCP option 121 rather than a cross-overlay `/32` route), and dynamic IMDS behavior (slow/partial/erroring responses, version negotiation under load) is unexercised. `vendor_data.json` support (cloud-init `convert_vendordata`: `cloud-init` key / bare-string, merged under `user_data`) is **unit-tested only**. `network_data.json` is carried through but not applied on Windows (v2). Do not ship to customer OpenStack until it has run against at least one real deployment.
