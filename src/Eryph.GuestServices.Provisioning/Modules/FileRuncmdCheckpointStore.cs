...
/// <summary>
/// File-backed runcmd checkpoint store. Layout mirrors the script
/// checkpoint store — runcmd.json lives next to the per-instance sem/
/// directory.
/// </summary>
public sealed class FileRuncmdCheckpointStore : IRuncmdCheckpointStore
{
    private readonly ILogger<FileRuncmdCheckpointStore> _logger;
    ...
    public async Task<RuncmdCheckpoint> LoadAsync(string instanceId, CancellationToken cancellationToken)
    {
        var path = ResolvePath(instanceId);
        if (!File.Exists(path))
            return RuncmdCheckpoint.Empty;
        ...
        try
        {
            ...
            var parsed = await JsonSerializer.DeserializeAsync(
                stream,
                RuncmdCheckpointJsonContext.Default.RuncmdCheckpoint,
                cancellationToken).ConfigureAwait(false);
            return parsed ?? RuncmdCheckpoint.Empty;
        }
        catch (Exception ex) when (ex is JsonException or IOException)
        {
            _logger.LogWarning(ex,
                "Runcmd checkpoint at {Path} is unreadable; starting from empty checkpoint",
                path);
            return RuncmdCheckpoint.Empty;
        }
    }

    public async Task SaveAsync(string instanceId, RuncmdCheckpoint checkpoint, CancellationToken cancellationToken)
    {
        var path = ResolvePath(instanceId);
    ...
    private string ResolvePath(string instanceId) =>
        Path.Combine(_root, "instance", SanitizeForPath(instanceId), "runcmd.json");
