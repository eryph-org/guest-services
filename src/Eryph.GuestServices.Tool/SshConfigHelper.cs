...
public static class SshConfigHelper
{
    ...
    // Rewrites a Windows absolute path into a form that BOTH OpenSSH builds
    // accept, so a single generated config serves native ssh.exe and the
    // MSYS/Git-Bash ssh used by embedded shells alike. Native Windows OpenSSH
    // understands 'C:\...'/'C:/...'; the MSYS OpenSSH that Git-Bash ships only
    ...
    // forward-slashed so native ssh.exe keeps working.
    internal static string ToPortableSshPath(string path)
    {
        // Collapse to a single separator first so the profile-prefix test is not
        // defeated by a forward slash at the boundary (e.g. a '--identity
        // C:/Users/Jane/...' value): without this such a path would miss the
        // '~'-anchoring and only get forward-slashed.
        var normalized = path.Replace(Path.AltDirectorySeparatorChar, Path.DirectorySeparatorChar);
        var profile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        string portable;
        if (!string.IsNullOrEmpty(profile)
            && (normalized.Equals(profile, StringComparison.OrdinalIgnoreCase)
                || normalized.StartsWith(profile + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase)))
        {
            var relative = normalized[profile.Length..].TrimStart(Path.DirectorySeparatorChar);
            portable = relative.Length == 0 ? "~" : "~/" + relative;
        }
        else
        {
            portable = normalized;
    ...
    // Builds the auto-generated include block placed in ~/.ssh/config. Kept as a
    // pure function so the portable path shape can be unit-tested without writing
    // to the developer's real ~/.ssh/config.
    internal static string BuildIncludeBlock() =>
        // Quote the include patterns: even though the '~'-anchored form has no
        // spaces, the quotes are harmless in both OpenSSH builds. In production the
        // config root is under LocalApplicationData, i.e. always under the profile,
        // so both includes '~'-anchor and load in native ssh.exe AND the MSYS ssh.
        // Deliberate limitation (no fallback): if the root is ever redirected
        // OUTSIDE the profile, '~' cannot name it — '~' is the profile by
        // definition — so the include is emitted as a forward-slashed absolute
        // path that native ssh.exe still loads but the MSYS glob() silently drops.
        // That off-profile case is knowingly native-only; we do not emit a second
        // '/c/...' cygdrive include to cover it.
        $"""
         {Border}
         # This part of the configuration has been auto-generated by
         # the eryph guest services and should not be modified manually.

         Include "{ToPortableSshPath(CatletSshConfigPath)}/*"
         Include "{ToPortableSshPath(VmSshConfigPath)}/*"

         {Border}
         """;

    // Test seam: overrides the ~/.ssh directory so the in-place update of the
    // user's main SSH config can be exercised against a temp file instead of the
    // developer's real ~/.ssh/config. Null in production.
    internal static string? UserSshPathOverride { get; set; }

    public static async Task EnsureSshConfigAsync()
    {
        var sshUserProfilePath = UserSshPathOverride ?? Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),
            ".ssh");

        var sshConfigPath = Path.Combine(sshUserProfilePath, "config");

        var config = BuildIncludeBlock();
        ...
        string keyFilePath)
    {
        ...
        // Quote the key path: a BYOK/profile path may contain spaces. Portabilize
        // it so the same config loads the key under both native ssh.exe and the
        // MSYS ssh used by embedded shells.
        builder.AppendLine($"    IdentityFile \"{ToPortableSshPath(keyFilePath)}\"");
        ...
        string? configurationName)
    {
        ...
        // Quote the selector values: a configuration/client id with a space (or
