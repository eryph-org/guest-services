...
// egs-tool catlet download-directory <CatletId> <SourcePath> <TargetPath> [--overwrite] [--recursive] [--identity <path>]
//
// Downloads a directory from a catlet over the eryph channel, sharing the
// transfer logic with the VM-level 'download-directory'.
public sealed class CatletDownloadDirectoryCommand : GuestTransferCommand<CatletDownloadDirectoryCommand.Settings>
{
    ...
    protected override Task<int> TransferAsync(SshSession session, Settings settings) =>
        GuestFileTransfer.DownloadDirectoryAsync(
