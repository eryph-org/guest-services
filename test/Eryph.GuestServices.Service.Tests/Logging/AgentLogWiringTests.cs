...
public sealed class AgentLogWiringTests : IDisposable
{
    ...
    [Fact]
    public void AddAgentLogging_writes_the_agent_log_via_serilog()
    {
        // Faithful to the composition roots (Program.cs / RunCommand), which call
        // builder.AddAgentLogging(). Serilog owns the pipeline and the file sink
        // (size/rolling/retention) is configured from the Serilog section; supply
        // a minimal one here. The helper injects AgentPaths.LogFile as the path.
