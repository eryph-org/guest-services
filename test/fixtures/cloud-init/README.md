...
| `eryph-yaml11-integer-forms.yaml` | **eryph contribution** (not copied) | Cloud-init's example corpus writes integers in plain decimal, so it never exercises the YAML 1.1 integer forms PyYAML SafeLoader still resolves: leading-zero octal (`0644` → 420), underscore separators (`1_000` → 1000), binary (`0b101` → 5), hex (`0x3E8` → 1000). YamlDotNet's YAML 1.2 parser would silently mis-read the octal forms as decimal and reject the underscore forms; this fixture pins parity with `safe_load` across `object?`, nested-record, and list-entry `int?` targets. Authored by the eryph guest-services project; no upstream copyright. |
